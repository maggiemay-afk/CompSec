...
public class PlaintextLoginModule implements LoginModule {
   ...
   private boolean checkPassword(String username, byte[] usernameAndPassword) throws NoSuchAlgorithmException {
	   ...
	   String alice = "DqaMyy1Avg7U1Ks7Bc1/ZDMAZXQvnIqlRfZmkznHSQA=";
	   String bob = "FsK3Vy5yCLMpdihxViSIiJhBgv84GZMZ0JoFD7I5lWk=";
	   byte[] aHash = alice.getBytes();
	   byte[] bHash = bob.getBytes();
	   
	   String digest = LoginUtil.getDigest(usernameAndPassword);
	   byte[] digestBytes = digest.getBytes();
	   
	   if (username.equals("alice")) {
		   if (Arrays.equals(aHash, digestBytes)) {
			   return true;
		   }
		   return false;
	   } else if (username.equals("bob/SERVICE")) {
		   if (Arrays.equals(bHash, digestBytes)) {
			   return true;
		   }
		   return false;
	   }
